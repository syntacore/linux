...
#define GET_DBTR_BIT(_target, _prefix, _bit_name) \
	test_bit(RV_DBTR_BIT(_prefix, _bit_name), &(_target))
...
#define RV_DBTR_GET_MC_EXEC(_t1) \
	GET_DBTR_BIT(_t1, MC, EXEC)

#define RV_DBTR_GET_MC_LOAD(_t1) \
	GET_DBTR_BIT(_t1, MC, LOAD)

#define RV_DBTR_GET_MC_STORE(_t1) \
	GET_DBTR_BIT(_t1, MC, STORE)

#define RV_DBTR_GET_MC6_EXEC(_t1) \
	GET_DBTR_BIT(_t1, MC6, EXEC)

#define RV_DBTR_GET_MC6_LOAD(_t1) \
	GET_DBTR_BIT(_t1, MC6, LOAD)

#define RV_DBTR_GET_MC6_STORE(_t1) \
	GET_DBTR_BIT(_t1, MC6, STORE)

#define RV_DBTR_GET_TYPE(_t1) ({ \
		((_t1) & RV_DBTR_BIT_MASK(MC, TYPE)) >> RV_DBTR_BIT(MC, TYPE); \
	})

#define RV_DBTR_GET_MC_SIZE(_t1) ({ \
	typeof(t1) t2 = t1; \
	(((_t2) & RV_DBTR_BIT_MASK(MC, SIZELO)) >> RV_DBTR_BIT(MC, SIZELO)) | \
		(((_t2) & RV_DBTR_BIT_MASK(MC, SIZEHI)) >> (RV_DBTR_BIT(MC, SIZEHI) - 2)); \
	})

#define RV_DBTR_GET_MC6_SIZE(_t1) ({ \
		((_t1) & RV_DBTR_BIT_MASK(MC6, SIZE)) >> RV_DBTR_BIT(MC6, SIZE); \
	})

#define RV_DBTR_GET_MC_CHAIN(_t1) ({ \
		((_t1) & RV_DBTR_BIT_MASK(MC, CHAIN)) >> RV_DBTR_BIT(MC, CHAIN); \
	})

#define RV_DBTR_GET_MC6_CHAIN(_t1) ({ \
		((_t1) & RV_DBTR_BIT_MASK(MC, CHAIN)) >> RV_DBTR_BIT(MC, CHAIN); \
	})

#define RV_DBTR_GET_MC_HIT(_t1) ({ \
		((_t1) & RV_DBTR_BIT_MASK(MC, HIT)) >> RV_DBTR_BIT(MC, HIT); \
	})

#define RV_DBTR_GET_MC6_HIT(_t1) ({ \
		((_t1) & RV_DBTR_BIT_MASK(MC, HIT)) >> RV_DBTR_BIT(MC, HIT); \
	})

#define RV_DBTR_CLEAR_MC_HIT(_t1) \
	CLEAR_DBTR_BIT(_t1, MC, HIT)

#define RV_DBTR_CLEAR_MC6_HIT(_t1) \
	CLEAR_DBTR_BIT(_t1, MC6, HIT)

typedef unsigned long riscv_dbtr_tdata1_mcontrol_t;
typedef unsigned long riscv_dbtr_tdata1_mcontrol6_t;

struct hw_breakpoint_regs {
	unsigned long tdata1;
	unsigned long tdata2;
	unsigned long tdata3;
};

struct hw_breakpoint_chain {
	unsigned long len;
	unsigned long idx;
	unsigned long fires;
	u32 type;
	struct hw_breakpoint_regs regs[];
};
...
int hw_dbtr_validate_type(unsigned long tdata1);
unsigned long hw_rawbp_parse_type(unsigned long tdata1);
unsigned long hw_rawbp_parse_size(unsigned long tdata1);
unsigned long hw_rawbp_parse_addr(unsigned long tdata2);
