...
static int mt_timer_enable(struct device_node *np)
{
	...
	/* keep bootloader settings if 'clock-frequency' is not specified */
	if (!of_property_read_u32(np, "clock-frequency", &rate) && rate > 0) {
		ret = regmap_update_bits(mt_regmap, MTIMER_REG_DIV, MTIMER_DIV_MASK,
					 rate / riscv_timebase - 1);
		if (ret)
			return ret;

		if (of_property_read_bool(np, "clock-external"))
			ret = regmap_write(mt_regmap, MTIMER_REG_CTRL,
	...
	ret = regmap_read(mt_regmap, MTIMER_REG_DIV, &reg);
	if (ret)
		return ret;

	rate = ((reg & MTIMER_DIV_MASK) + 1) * riscv_timebase;

	ret = regmap_read(mt_regmap, MTIMER_REG_CTRL, &reg);
	if (ret)
		return ret;

	ext = !!(reg & MTIMER_CTRL_SRC_EXT);

	pr_info("%pOFP: timebase frequency: %ld Hz, mtimer: %u Hz (%s)\n",
		np, riscv_timebase, rate, ext ? "external" : "internal");
...
static int __init mt_timer_init_dt(struct device_node *np)
{
	...
	if (rc) {
		pr_err("%pOFP: failed to enable timer: %d\n", np, rc);
		goto fail_iounmap;
	}
